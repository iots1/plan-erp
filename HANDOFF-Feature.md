...
4. ~~**§5 หนังสือรับรองหัก ณ ที่จ่าย** — บล็อกที่ `ap_invoices.supplier_tax_id`~~ ✅ **ปิดแล้ว 2026-09-09** — เพิ่ม snapshot ครบ 6 คอลัมน์ + `wht_income_type` + `POST /payment-entries/:id/wht-certificate` + e2e บนของจริง (`HANDOFF-Backlog-Reporting-Print-Tax.md` §5.6) · บรรทัดนี้เขียนไว้ 2026-09-08 แล้วงานแซงไปวันถัดมา
...
⚠️ **`HANDOFF-Postgresql.md` — ยังไม่มี backup เลยแม้แต่ชุดเดียว (§2.1) และ VM ทั้ง 7 อยู่บน hypervisor เดียว (§3.1)** · **ผู้ใช้ตัดสินใจ 2026-09-13 ว่ายังไม่ต้องทำ** เพราะยังเป็น environment ของ dev ไม่มีข้อมูลลูกค้าจริง (ตรวจแล้ว: ทุก `erp_*` รวม 77 MB, customers 3, receipts 11) — **อย่าเสนอซ้ำ** แต่กลับมาเป็นงานอันดับหนึ่งทันที เมื่อมีข้อมูลจริงเข้าระบบ/นัด go-live · เงื่อนไขครบ 4 ข้อและเหตุผลอยู่ที่ `HANDOFF-Postgresql.md` **§0**
...
บนโปรดักชันไม่รู้จักคอลัมน์ใหม่และไม่เดือดร้อน · ตอน deploy `migration:run` จะข้ามทั้งสามเพราะบันทึกว่า
executed แล้ว (นี่คือเหตุผลที่ grant migration รอบนี้ปลอดภัยจากกับดักลำดับ deploy — ดู `.claude/rules/permissions.md`)
...
**smoke พิสูจน์ VAT ครบทั้งสองทิศแล้ว 2026-09-20** — `apps/sales-bc/test/smoke/pos-sales.smoke.mjs`
ขายของจริง ออกใบกำกับจริง แล้ว void คืน (เลขใบกำกับยังถูกใช้ไปแล้วถาวร คืนไม่ได้ตามกฎ §86/4 —
นั่นคือราคาที่ต้องจ่ายเพื่อพิสูจน์เส้นนี้)

เดิม dataset มีสินค้าที่มีสต็อกอยู่ตัวเดียวและเป็น `exempt` (ปุ๋ย ยกเว้น VAT จริงตามกฎหมาย)
assertion ที่พิสูจน์การถอด VAT จึงไม่เคยรัน · **แก้แล้วด้วยการรับ `EQP-TOOL-T01-M`
(standard-rated, 20 หน่วย) เข้าสต็อกผ่านเส้นทางจริง A2→A3** ไม่ใช่ยัดล็อตเข้า DB ตรง ๆ
เพราะล็อตต้องมี `import_cost` จริงถึงจะคิด COGS ได้ · ผลที่ได้ตอนนี้:

| ประเภทเอกสาร | ราคาที่คีย์ | ผลลัพธ์ |
|---|---|---|
| `ABB_TAX_INVOICE` | 100 (ราคาป้าย) | net **93.4579** + vat **6.5421** = **100** — ถอด VAT ออกจากราคา |
| `FULL_TAX_INVOICE` | 100 (ก่อน VAT) | total **107** — บวก VAT ทับ |

สองแถวนี้คือหัวใจของฟีเจอร์ และตอนนี้มีเทสต์จริงคุมทั้งคู่ · suite ยัง log WARNING ไว้เผื่อวันหนึ่ง
สินค้าที่ standard-rated หมดสต็อกแล้วมันเงียบ ๆ ตกไปใช้กิ่งที่พิสูจน์อะไรไม่ได้อีก

**กับดักที่เจอตอนรับของ** — ใบรับสินค้าลงได้เฉพาะคลังที่เป็น **stock leaf**
(`is_group=false`, `warehouse_type=stock`) · หยิบ `warehouses[0]` มาใช้ตรง ๆ จะได้ node ที่เป็น
group แล้วโดนปฏิเสธ
...
สิ่งที่ตอบ §5 ให้เกือบหมดและปลดตัวบล็อกที่ค้างมาตั้งแต่ 2026-09-13

**ผลที่ไม่คาดคิดจากไฟล์นั้น — ชื่อบรรทัดบนงบไม่ต้องเดาเลย**: ไล่เทียบทุกบรรทัดกับผังบัญชีที่ seed ไว้แล้ว
**ตรงกันทุกบรรทัด** (`รายได้จากการให้บริการ` = 4100-02, `ค่าจ้างทำโปรเจค` = 5230-00,
`เงินสมทบกองทุนประกันสังคม` = 5412-12, `เบี้ยปรับเงินเพิ่ม` = 5490-02, …) — ผังมาตรฐานที่ seed ไว้ถอดมาจาก
ระบบบัญชีตัวเดียวกับที่ออกไฟล์นี้ · ข้อกังวลเดิมที่ว่า "ถ้าไม่มีแบบฟอร์มจริง ชื่อบรรทัดจะเป็นการเดา"
จึงหมดไป ชื่อมาจาก `gl_accounts.name_th` ตรง ๆ

**สิ่งที่ทำ** (รายละเอียดเต็มทุกการตัดสินใจ: `HANDOFF-Financial-Statements.md` **§10**):

- **โมดูลใหม่ `financial-statement`** ใน finance-bc — `GET /income-statements` (ช่วงเวลา) +
  `GET /balance-sheets` (ณ วันที่) · ไม่มีตารางของตัวเอง คำนวณสดทุกครั้ง
- **คอลัมน์ใหม่ `gl_accounts.statement_section`** (enum 4 ค่า, nullable) — ตัวที่ทำให้คำนวณ**กำไรขั้นต้น**ได้
  เพราะ `account_type` บอกได้แค่ว่าบัญชีอยู่งบไหน ไม่ได้บอกว่าอยู่บรรทัดไหน (ผังเก็บต้นทุนขาย 5100/5200 กับ
  ค่าใช้จ่าย 5300/5400/5500 ไว้ใต้ราก `5000-00` เดียวกัน) · **ไม่ใช้กฎจากรหัสบัญชี** เพราะลูกค้าเปลี่ยนรหัสได้
  ตามดีไซน์ของ chart-of-accounts · ติดป้าย 7 บัญชีคุม → จัดกลุ่มบัญชีกำไรขาดทุนทั้ง 124 บัญชี
- **สร้างบน `TrialBalancesService.build()` ไม่ query ledger เอง** — ทำให้ "งบต้องตรงกับงบทดลอง" เป็นจริง
  เชิงโครงสร้าง ไม่ใช่แค่ตั้งใจ
- **smoke ไฟล์ใหม่** ที่เทียบงบกับ `/trial-balances` ของช่วงเดียวกันทุกหมวด + เช็คว่าไม่มีบัญชีไหนหายไป
  + `profit_for_period` ของงบดุลต้องเท่ากับ `net_profit` ของงบกำไรขาดทุนปีเดียวกัน

**2 กับดักที่เจอระหว่างทาง (ทั้งคู่จับได้ด้วย unit test ไม่ใช่ตอน review)**:

1. **flip เครื่องหมายต้องตาม `account_type` ไม่ใช่ `normal_balance`** — ที่ออกแบบไว้เดิม (§3.3 ของ handoff นั้น)
   ผิด: `4100-03 รับคืนสินค้า` เป็น REVENUE ที่ `normal_balance = DEBIT` ถ้า flip ตาม normal_balance
   จะกลายเป็น**บวกรับคืนสินค้าเข้ายอดขาย** · docblock ของ `NormalBalance` เตือนเรื่องบัญชีปรับมูลค่าไว้อยู่แล้ว
   แต่เตือนคนละทิศกับที่ต้องใช้ตรงนี้
2. **ยอดห้ามไต่ขึ้นไปหาบรรพบุรุษที่ "หมวดตรงกันเพราะตกหมวดสำรอง"** — กติกาแรกที่เขียนทำให้ราก `5000-00`
   กลืน subtree ต้นทุนขายเข้ามารวมในหมวดค่าใช้จ่ายด้วย นับซ้ำข้ามหมวดทันที · แก้เป็น "ไต่ผ่านบรรพบุรุษที่
   **เป็นเจ้าของหมวดนั้นจริง** (ติดป้ายเอง/สืบทอดจากที่ติดป้าย) เท่านั้น"

**ยืนยันด้วยการรันจริงทุกข้อ**: `pnpm verify finance-bc` เขียวครบ 6 ขั้น · `pnpm typecheck` ผ่าน ·
`NODE_ENV=local npx jest` = **1834/1834 (138 suites)** · finance-bc e2e 36/36 · smoke 6 ไฟล์ผ่านหมด
(`assets 900, L+E 900, difference 0, is_balanced=true`) · `migration:generate` รายงาน **No changes** ·
`permissions:sync` เพิ่ม 2 สิทธิ์ (`income_statement:view`, `balance_sheet:view`) แล้ว grant migration รันจริง

⚠️ **ฝั่ง P&L ของคลัสเตอร์นี้ยังแทบว่าง** (EXPENSE ที่มีรายการ 1 บัญชี ยอดสุทธิ 0, REVENUE ยังไม่มีเลย)
ตามที่ handoff §1.2 บอกไว้ — smoke จึงยืนยัน**ความสอดคล้อง**ไม่ใช่ตัวเลข ส่วนตัวเลขพิสูจน์ด้วย unit test
...
`HANDOFF-Financial-Statements.md` §10.5
...
**P4 ต่อทันทีในรอบเดียวกัน — 12/25** (`HANDOFF-Document-Print-Pipeline.md` §7.4): ไม่ได้เขียน HTML
12 ไฟล์ แต่แยกไฟล์ตามความต่าง **เชิงโครงสร้าง** 4 ไฟล์ (ใบกำกับภาษี / เอกสารไม่มี VAT / ใบลดหนี้-
เพิ่มหนี้ / ใบเสนอราคา) แล้วให้ชื่อเอกสารกับโน้ตภาษีมาเป็น params ตามแพตเทิร์น `wht_line` ของ P3 ·
**ใบลดหนี้/เพิ่มหนี้ต้องแก้ mapper จริง**: §86/10 ขอสามตัวเลข (มูลค่าเดิม/ที่ถูกต้อง/ผลต่าง) แต่ระบบ
เก็บแค่ผลต่าง — ต้องโหลด relation `reference_receipt` ตอน print แล้วคำนวณ · **สาเหตุการออกเอกสาร
ยังไม่มีคอลัมน์ของตัวเอง** ใช้ `remark_th` ไปก่อน (ถ้าจะทำให้ถูกเต็มรูปต้องคุยเรื่อง schema ก่อน) ·
เหลืออีก 13 ใบที่**ยังไม่มี `POST /:id/print` เลย** — งานต่อใบคือ endpoint+mapper+smoke ของ BC
เจ้าของ ไม่ใช่แค่ HTML

**รอบสอง (ตามที่ผู้ใช้สั่งให้เน้นใบหลัก)** — 3 อย่าง:
1. **`adjustment_reason` เป็นคอลัมน์จริงแล้ว** (ไม่ใช้ `remark_th` แทนอีก) — บังคับกรอกเมื่อ
   `document_type=CREDIT_NOTE|DEBIT_NOTE` ตาม §86/10 และถูกบังคับเป็น `null` สำหรับอีก 6 ประเภท ·
   `update()` ประเมินใหม่ทุกครั้งเพราะ `document_type` ยังเปลี่ยนได้ตอน DRAFT
2. **กรอก `company_profiles` ด้วยข้อมูลตัวอย่าง** ตามที่ผู้ใช้สั่ง (ผู้ใช้จะบันทึกข้อมูลจริงทีหลัง) —
   `tax_id = 0000000000000` และที่อยู่มีคำว่า "(ข้อมูลตัวอย่าง — รอบันทึกข้อมูลจริง)" ติดอยู่บนตัว
   เอกสารเอง เพื่อไม่ให้เผลอส่งออกไปโดยคิดว่าเป็นข้อมูลจริง · เอกสารที่พิมพ์มีบล็อกผู้ออกครบแล้ว
3. **`POST /goods-receipts/:id/print`** (inventory-bc) — เทมเพลต `goods-receipt.html` เป็นใบเดียวที่มี
   **คอลัมน์ล็อต/วันหมดอายุ** และพิมพ์จำนวนสองชุด (หน่วยสต็อก + ที่ผู้รับกรอก) · **GRN ไม่ snapshot
   อะไรเลยนอกจาก id** — เพิ่มชื่อ/SKU สินค้าบนบรรทัด และ `po_number`/ชื่อผู้ขายบนหัวเอกสาร ได้ฟรีจาก
   lookup ที่ validate อยู่แล้วทั้งคู่ ไม่ต้องยิงข้าม BC ตอนพิมพ์
4. **`POST /purchase-orders/:id/print`** (supplier-bc) — endpoint ใหม่ตัวแรกหลัง P2 + เทมเพลต
   `purchase-order.html` (ไม่มี VAT เลย เพราะภาษีจบที่ใบกำกับของผู้ขาย) · **เจอว่า
   `purchase_order_items` ไม่เคย snapshot ชื่อ/SKU สินค้า** — ใบที่พิมพ์จะไม่มีรายการสินค้า · เพิ่ม 3
   คอลัมน์ + ให้ RPC เดิมที่ validate สินค้าอยู่แล้วคืนค่ามาเก็บ (เดิมทิ้งผลลัพธ์ไปเปล่า ๆ)

**ยืนยัน** — `pnpm verify finance-bc`/`sales-bc`/`supplier-bc`/`report-bc` เขียวครบ · smoke ยิงพิมพ์จริงผ่าน
RMQ → Gotenberg → storage แล้วเช็คกลับว่าเป็น `banded` v3, snapshot มี `items[]`, ลายน้ำตรงสถานะ,
PDF จริง ~70KB · **พิมพ์ใบลดหนี้จริงด้วย** แล้วเช็คว่า `original_total` ไม่ว่าง (พิสูจน์ว่า relation
ถูกโหลด — ถ้าลืมขอจะว่างเงียบ ๆ) และเลขเดินถูก `original 360.80 → corrected 270.60` · render ด้วย
Chrome ในเครื่องแล้วดูหน้ากระดาษจริงทั้ง 4 แบบ (34 บรรทัด → 3 หน้า)
