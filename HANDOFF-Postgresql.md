...
> **อัปเดต 2026-09-13 — ยังเป็น environment ของ dev ผู้ใช้ตัดสินใจว่ายังไม่ต้องทำตอนนี้ · ดู §0**
> เนื้อหาถาวรอยู่ที่ `postgresql-pgpool-cluster-guide.html` / `ha-failover-operations-guide.html` / `runbook-*.html`

---

## 0 · สถานะการตัดสินใจ — ยังเป็น dev, เลื่อนออกไปก่อน (2026-09-13)

**ผู้ใช้ตัดสินใจว่างานในไฟล์นี้ยังไม่ต้องทำตอนนี้ เพราะคลัสเตอร์ยังอยู่ในช่วง development**
บันทึกไว้ตรงนี้เพื่อให้ session ถัดไปไม่ต้องเสนอซ้ำ และเพื่อให้รู้ว่า **เมื่อไหร่ที่การตัดสินใจนี้
หมดอายุ**

**ข้อมูลที่มีอยู่จริงวันนี้** (ตรวจด้วย query จริง 2026-09-13) — ทุก `erp_*` database รวมกัน **77 MB** ·
customers 3 · receipts 11 · ledger_entries 133 · products 86 · users 14 · ทั้งหมดเป็นข้อมูลทดสอบ/ตั้งต้น
**ยังไม่มีข้อมูลของลูกค้าจริงสักราย** — ถ้าคลัสเตอร์หายทั้งชุดวันนี้ สิ่งที่เสียคือเวลา seed ใหม่
ไม่ใช่ข้อมูลธุรกิจที่สร้างใหม่ไม่ได้ นั่นคือเหตุผลเดียวที่ §2.1 รอได้

**สิ่งที่การตัดสินใจนี้ไม่ได้แปลว่า** — ไม่ได้แปลว่า §2.1 ผิดหรือเล็กลง · เนื้อหาทุกข้อในไฟล์นี้ยังจริง
ทั้งหมด และ **"dev" ในที่นี้ไม่ได้หมายถึงเครื่องทิ้งขว้าง**: `.env` ของเครื่อง dev กับของ production
ชี้ไปที่คลัสเตอร์ **ตัวเดียวกัน** (`HANDOFF-Feature.md` §4 กับดัก #10) ดังนั้นวันที่ข้อมูลจริงเข้ามา
จะไม่มีอะไรเปลี่ยนในทางเทคนิคเลย — เปลี่ยนแค่ราคาของความผิดพลาด

**เงื่อนไขที่ทำให้ต้องกลับมาทำทันที** (ข้อใดข้อหนึ่งพอ):

1. มีข้อมูลของลูกค้าจริงเข้าระบบ แม้แต่รายเดียว
2. มีการนัดวัน go-live / UAT กับลูกค้า
3. เริ่มมีคนนอกทีมพัฒนากรอกข้อมูลเข้าระบบ (ข้อมูลที่ไม่มีใคร seed ซ้ำให้ได้)
4. ขนาดข้อมูลโตจนการ seed ใหม่ไม่ใช่ทางออกที่ยอมรับได้อีกต่อไป

ถึงตอนนั้นให้เริ่มที่ **§2.1 ก่อนทุกข้อ** (backup + พิสูจน์ว่า restore ได้จริง) แล้วค่อยไล่ §2.2–§2.10 ·
งานที่เหลือในไฟล์นี้ (ซ้อม failover, switchover, เพดาน connection, alert) เป็นเรื่องความพร้อมใช้งาน
ซึ่งรอ go-live ได้ แต่ **backup เป็นเรื่องการมีอยู่ของข้อมูล ซึ่งรอไม่ได้หลังมีข้อมูลจริงแล้ว**
...
> **เลื่อนออกไปก่อนตามการตัดสินใจ 2026-09-13 (ดู §0)** — ยังเป็น environment ของ dev
> ไม่มีข้อมูลลูกค้าจริง · ข้อความข้างล่างนี้ยังจริงทุกข้อ และจะกลับมาเป็นงานอันดับหนึ่งทันที
> ที่ข้อมูลจริงเข้าระบบ
