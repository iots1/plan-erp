...
## 10.1 · ส่วนขยายที่มาทีหลัง — `statement_section` (2026-09-20)

งบการเงิน (D3) เพิ่มคอลัมน์ `gl_accounts.statement_section` เข้ามาในตารางนี้ พร้อมช่องในฟอร์มสร้าง/แก้ไข
ของหน้า `gl-accounts` · เป็นแกนที่ **สอง** ของผังบัญชีต่อจาก `account_role`: role ตอบว่า "posting rule
ตัวไหนค้นหาบัญชีนี้" ส่วน section ตอบว่า "งบกำไรขาดทุนพิมพ์บัญชีนี้ไว้บรรทัดไหน" — คนละคำถาม
คนละกติกา (role หนึ่งค่าต่อหนึ่งบัญชี · section ใส่ที่บัญชีคุมแล้วลูกสืบทอดทั้ง subtree)

เหตุผลที่ต้องเป็นคอลัมน์ ไม่ใช่กฎจากรหัสบัญชี: ทั้งไฟล์นี้ตั้งอยู่บนสมมติฐานว่า**ลูกค้าเปลี่ยนรหัส/ย้าย/
เปลี่ยนชื่อผังได้เอง** กฎแบบ `51xx` = ต้นทุนขาย จึงผิดเงียบ ๆ ทันทีที่มีคนใช้สิทธิ์นั้น ·
รายละเอียดเต็ม: `HANDOFF-Financial-Statements.md` §10.2
