...
>
> **สถานะ 2026-09-09: P0–P4 ✅ — พิมพ์ได้ครบ 25/25 ใบ (ดู §7)** · `company_profiles` บันทึกข้อมูล
> ผู้ประกอบการจริงของ deployment นี้แล้ว (ค่าอยู่ใน DB เท่านั้น — **ไม่เขียนลงเอกสารสาธารณะ** ตาม
> root `CLAUDE.md`) · `tax_id` ยังเป็น placeholder ตามที่เจ้าของสั่ง — ใช้เลข 9 ทั้ง 13 ตัว ซึ่ง
> **check digit ไม่ผ่าน** (ตัวสุดท้ายต้องเป็น 4) จึงไปตรงกับทะเบียนจริงของใครไม่ได้ ต่างจากเลขที่ดู
> สมจริงกว่า · เต็ม 13 หลักเพื่อให้ layout ที่พิมพ์ออกมาเป็นของจริง · **ต้องใส่เลขจริงก่อนออกใบกำกับ
> ภาษีให้ลูกค้า** (§86/4(2)) — พิมพ์ **ใบกำกับภาษีเต็มรูป** และ
...
| **P4** | ขยายให้ครบ 25 เอกสาร (mapper + HTML ทีละใบ) | ~0.5 วัน/ใบ | ✅ **25/25 · 2026-09-09** — ทุก `document_types` code มี endpoint + เทมเพลต `banded` จริง · 11 endpoint `POST /:id/print` ใน 5 BC · ดู §7.4 |
...
**รอบสาม — ใบรับสินค้า (inventory-bc), 1 code**

`POST /goods-receipts/:id/print` (`goods_receipt:print`) · `goods-receipt.html` เป็น layout เดียวที่มี
**คอลัมน์ล็อตและวันหมดอายุ** เพราะหน้าที่ของใบรับสินค้าคือพิสูจน์ว่า *ของอะไรเข้ามาจริง* ไม่ใช่ยอดเงิน ·
พิมพ์จำนวนสองชุด: `qty` (หน่วยนับสต็อก ซึ่งเป็นตัวที่ lot/over-receipt bound อ่าน) และ `qty_entered`
(ที่ผู้รับกรอกก่อนแปลงหน่วย) — ใบที่โชว์แต่ตัวแปลงแล้วเอาไปทานกับใบส่งของไม่ได้ · ไม่มีมิติภาษีและ
ส่วนลดเลย และมีข้อความกำกับว่าไม่ใช่ใบกำกับภาษี/ไม่ใช่หลักฐานการชำระเงิน

**ช่องว่างที่เจอตอนทำ (แก้แล้ว)** — GRN ไม่ snapshot อะไรเลยนอกจาก id: ทั้งชื่อสินค้า, เลขที่ PO และ
ชื่อผู้ขาย · เพิ่ม `product_sku`/`product_name_th/en` บน `goods_receipt_items` (ได้ฟรีจาก
`resolveTransactableProduct()` ที่อ่าน product มา validate อยู่แล้ว) และ `po_number`/`supplier_name_th/en`
บน `goods_receipts` (ได้ฟรีจาก PO lookup ที่ validate `po_id` อยู่แล้ว — RPC ตัวนั้นคืน entity เต็ม
ชื่อผู้ขายจึงข้ามมาอยู่แล้ว แค่ interface ฝั่งผู้อ่านไม่ได้ประกาศไว้) · **ไม่ต้องยิงข้าม BC ตอนพิมพ์เลย**

**รอบสี่ — สายขายที่เหลือ (sales-bc), 3 code** · proxy มีอยู่แล้วตั้งแต่ P2 จึงเหลือแค่ mapper +
endpoint + เทมเพลตต่อใบ:

- **ใบสั่งขาย** — layout เดียวกับใบเสนอราคา แต่ประกาศบนหน้าเอกสารว่า **ไม่ใช่ใบกำกับภาษี** เพราะใบกำกับ
  จะออกตามจำนวนที่ส่งมอบจริงในภายหลัง (คนอ่านที่เข้าใจผิดว่าใบสั่งขายเอาไปเคลมได้คือปัญหาจริง)
- **ใบส่งของ** — **ไม่มีตัวเลขเงินเลยทั้งใบ**: `DeliveryNoteItem.line_total` เป็น `0` โดยดีไซน์ (entity
  เขียนไว้เองว่าสืบทอด base มาแต่ไม่ใช้คอลัมน์เงิน) และของที่คนขับรถยื่นให้ลูกค้าคือรายการสินค้า ไม่ใช่
  ใบเรียกเก็บเงิน · คำอธิบายสินค้าดึงจาก**บรรทัดใบสั่งขาย**ที่ใบส่งของนั้นไปเติมเต็ม (join ใน database
  ของ sales-bc เอง ไม่ใช่ข้าม BC ที่ RULE · SNAPSHOT ห้าม) — เก็บ snapshot ไว้ที่เดียวไม่ต้องคัดลอกซ้ำ
  ลงทุกใบส่งของ
- **ใบรับคืนสินค้า** — `return_reason` อยู่เหนือตัวเลข ไม่ใช่ซ่อนใน remark (ใบคืนที่ไม่บอกสาเหตุคือใบที่
  ไม่มีใคร reconcile กับใบลดหนี้ที่ตามมาได้) และประกาศว่า **ไม่ใช่ใบลดหนี้** — §86/22 ต้องออกเอกสารแยก
  พร้อมเลขที่ของตัวเอง · คำอธิบายไล่ผ่าน `delivery_note_item.sales_order_item` เพราะการคืนมีได้เฉพาะ
  ของที่ส่งไปแล้ว ซึ่งเป็นสายที่ตัวเอกสารเองบรรยาย

smoke ของ sales-bc เปลี่ยนชื่อเป็น `documents-print.smoke.mjs` แล้วยิงพิมพ์ทั้ง 4 ใบในไฟล์เดียว ·
ใบที่ยังไม่มีข้อมูลในคลัสเตอร์ (ใบรับคืน) **skip พร้อม log** ไม่ใช่ fail — suite นี้วิ่งบนข้อมูลจริงที่มีอยู่

**รอบห้า — 5 ใบสุดท้าย (finance-bc 4 + inventory-bc 1)** · ปิดครบ 25/25:

- **ใบวางบิล** — **บรรทัดเป็นเอกสาร ไม่ใช่สินค้า**: หนึ่งแถวต่อหนึ่งใบกำกับที่ถึงกำหนด (เลขที่/วันที่/
  ครบกำหนด/ยอด) · ไม่มีบรรทัด VAT เพราะภาษีระบุอยู่บนใบกำกับแต่ละฉบับแล้ว การพิมพ์ซ้ำที่นี่คือการเชิญ
  ให้เคลมสองครั้ง
- **ใบสำคัญรับ/จ่ายเงิน** — หนึ่งแถวต่อ allocation พร้อมยอดหัก ณ ที่จ่ายของแถวนั้น (สิ่งที่ทำให้ใบสำคัญ
  reconcile ได้: ต้องเห็นว่าเงินก้อนเดียวไปตัดใบไหนบ้าง เท่าไร) · เงินเข้า/เงินออกใช้เทมเพลตเดียวกันแต่
  **ชื่อเอกสารต่างกันตาม `payment_type`** — พิมพ์ "ใบสำคัญรับเงิน" ทับรายการจ่ายคือวิธีที่ลายเซ็นไปอยู่
  ผิดฝั่งของธุรกรรม · `unallocated_amount` พิมพ์แม้เป็น 0 เพราะเงินที่รับแล้วยังไม่ผูกเอกสารคือยอดที่
  ต้องมีคนตาม
- **ใบตั้งหนี้ผู้ขาย / ใบลดหนี้จากผู้ขาย** (2 code, เทมเพลตเดียว) — **นี่คือบันทึกเอกสารของผู้ขาย ไม่ใช่
  เอกสารที่เราออก** · พิมพ์ `vendor_document_number` คู่กับเลขที่ของเรา (คนถือใบกระดาษต้องหาแถวนี้เจอ
  และคนถือใบพิมพ์นี้ต้องหาใบกระดาษเจอ) และพิมพ์ `vendor_tax_invoice_type` เพราะมันเป็นตัวตัดสินว่า
  ภาษีซื้อใบนั้นใช้สิทธิได้หรือไม่ (§82/5, §86/6)
- **ใบส่งคืนสินค้า** — ใบรับสินค้ากลับด้าน: คอลัมน์ล็อตเหมือนกัน (เอกสารนี้ออกไปพร้อมของ ผู้ขายต้อง
  แกะได้ว่าล็อตไหนมาจากส่งมอบครั้งใด) + สาเหตุการส่งคืน + ประกาศว่า **ไม่ใช่ใบลดหนี้** เพราะใบลดหนี้เป็น
  เอกสารที่ผู้ขายออกให้เรา

**สรุปทั้งเฟส** — 25 code · 11 endpoint · 12 ไฟล์เทมเพลต (ครอบครัวเดียวกันใช้ไฟล์ร่วมกัน ต่างกันด้วย
params ตามแพตเทิร์นของ P3) · 5 BC (sales/finance/supplier/inventory เป็นผู้เรียก, report-bc เป็นผู้
เรนเดอร์) · smoke ครบ 5 BC โดยแต่ละไฟล์ยิงพิมพ์จริงทุกใบที่ BC นั้นออก และ **skip พร้อม log** เมื่อ
ยังไม่มีข้อมูลของประเภทนั้นในคลัสเตอร์ (วันนี้: ใบรับคืนสินค้า, ใบส่งคืนสินค้า)

**บรรทัดติดต่อของผู้ออกเอกสารประกอบฝั่ง server** (`company_contact_line`) — เดิมทุกเทมเพลตเขียน
`โทร. {{company_phone}} · {{company_email}}` ตรง ๆ ซึ่งพอ profile มีเบอร์แต่ไม่มีอีเมล ก็เหลือ `·`
ค้างท้ายบรรทัดบนเอกสารทั้ง 25 ใบ · engine ไม่มี conditional จึงย้ายไปประกอบที่ `mergeParams()` แบบเดียว
กับ `wht_line`/`draft_watermark_text` — เจอตอนพิมพ์ใบจริงใบแรกหลังใส่ข้อมูลบริษัท ไม่ใช่ตอนรีวิวโค้ด

**ยังไม่ได้ทำ (P5 + ที่เหลือ)** (P5 = ต้นฉบับ/สำเนา/ใบแทน ตาม §86/12 ซึ่งยัง**รอฝ่ายบัญชียืนยัน** และฟอร์ม ภ.พ.30 · แถว `invoice` →
`PAYMENT_RECEIPT` ยังเป็นขยะจากระบบอื่นตาม §8.2 ข้อ 4 (ยังไม่ลบ) · ทุกเทมเพลตยังไม่มีใครทดลองพิมพ์
ลงกระดาษจริงจากเครื่องพิมพ์ออฟฟิศ — ตรวจด้วย PDF/หน้าจอเท่านั้น
