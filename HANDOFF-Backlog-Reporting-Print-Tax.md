...
**ยังเหลือ (ต้องรอข้อมูลจริง/คนตัดสิน)**:

1. **happy path ยังไม่เคยยิงบนเอกสารจริง** — คลัสเตอร์ยังไม่มี payment แบบ PAY+SUBMITTED ที่หักภาษี
   และไม่มีใบตั้งหนี้ไหนผูก `wht_config_code` · **จงใจไม่ปั้นให้** เพราะ GRN ที่ยังตั้งหนี้ไม่ครบเป็น
   **ค่าสินค้า** ซึ่งหักภาษี ณ ที่จ่ายไม่ได้ตามหลัก — การใส่ WHT ลงไปคือลง ledger ผิดหลักบัญชี ไม่ใช่
   test fixture · smoke จึงยืนยัน (ก) guard คืน 400 จริงบนเอกสารที่มี และ (ข) **เรนเดอร์เทมเพลต ทวิ 50
   ผ่าน Gotenberg จริงทุกรอบ** ซึ่งเป็นรอยต่อเดียวที่ unit test เอื้อมไม่ถึง (params key ที่เทมเพลตไม่
   อ่าน = หน้ากระดาษว่างตรงที่ควรมีเลข) · เมื่อมีการจ่ายเงินหักภาษีจริง smoke สลับไปพิมพ์ของจริงเอง
2. **เลขที่หนังสือรับรอง** — v1 ใช้เลขใบสำคัญจ่าย และตั้ง `has_running_number: false` ไว้ · ถ้าฝ่ายบัญชี
   ต้องการเลขชุดของตัวเอง (สำหรับ "ลำดับที่" ในแบบ ภ.ง.ด.) ต้องเพิ่ม counter + ขั้นออกเลข
3. **ผู้ถูกหักที่เป็นบุคคลธรรมดา** — `party_is_juristic` มาจาก `suppliers.supplier_type` ซึ่งมีค่า
   `company`/`individual` อยู่แล้ว แต่ยังไม่มีใครตรวจว่าข้อมูลผู้ขายในระบบกรอกถูกจริง ถ้ากรอกผิด
   หนังสือรับรองจะระบุแบบยื่นผิด
