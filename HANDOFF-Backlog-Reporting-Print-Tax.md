...
4. ~~**หน้าจอ/PDF ฟอร์มยื่นภาษีจริง** (แบบ 16 ช่อง)~~ ✅ **ทำแล้ว 2026-09-12 — ดู §4.6**

### 4.6 ผลตรวจสอบ — 2026-09-12 ✅ **ฟอร์ม ภ.พ.30 พิมพ์ได้จริง (implement + verify + smoke)**

`POST /finance-bc/v1/vat-returns/print` (`vat_return:print`) · body `{ year, month, locale?,
idempotency_key?, print_reason? }` · ตัวเลขสร้างจาก `VatReturnsService.build()` ตัวเดิม ไม่ได้คำนวณซ้ำ
คนละทาง แล้ว map ลงช่องด้วย `vat-return-print-params.util.ts` (pure function, unit test 7 เคส)

**เอกลักษณ์ของงานนี้ที่เอกสารอื่นไม่มี — ไม่มีแถวเอกสารให้พิมพ์**

`document_prints.source_document_id` เป็นคอลัมน์ `uuid` และ pipeline นับ `copy_number` ต่อ id นั้น
แต่ ภ.พ.30 ไม่มีตารางของตัวเอง (v1 คำนวณสด) · ทางออก: **UUIDv5 ของงวด** (`vat-return:YYYY-MM` ภายใต้
namespace คงที่) — พิมพ์เดือนเดิมซ้ำจึงเดินเลขสำเนาต่อกันในสายเดียว ไม่ใช่ขึ้น "ต้นฉบับ" ใหม่ทุกครั้ง
(ถ้าใช้ id สุ่ม ลำดับสำเนาจะไม่มีความหมาย ถ้าใช้ค่าคงที่ตัวเดียว ทุกเดือนจะกลายเป็นสายเดียวกันหมด) ·
`source_document_number` = `PP30-YYYY-MM`

**ช่องที่จงใจพิมพ์ว่าง** — 10 (ภาษีที่ชำระเกินยกมา), 13 (เงินเพิ่ม), 14 (เบี้ยปรับ), 15, 16 · ระบบ
ไม่เก็บยอดยกมา (ข้อ 3 ด้านบน) และไม่รู้วันที่ยื่นจริงซึ่งเป็นตัวกำหนดเงินเพิ่ม/เบี้ยปรับ การพิมพ์
`0.00` ลงไปคือการยืนยันสิ่งที่ไม่มีใครตรวจ — หลักการเดียวกับ `payee_tax_id_label` ของ ทวิ 50 · ช่อง
11/12 คำนวณโดยถือว่าช่อง 10 เป็นศูนย์ **และพิมพ์ข้อความกำกับข้อสมมตินี้บนใบเอง** ไม่ปล่อยให้เดา

**ผลพลอยได้ที่ใช้ได้กับทุกแบบ ภ.ง.ด. ต่อไป** — `company_tax_id_digits` (13 ช่อง เติมว่างถ้าเลขสั้น)
ประกอบที่ `mergeParams()` ของ report-bc เพราะ `{{key}}` แทนที่ได้แค่ข้อความ ตัดสตริงไม่ได้ และถ้าไป
ตัดด้วย JS ในหน้าเว็บจะไปชนกับ `__PAGINATION_DONE` ที่ Gotenberg ใช้เป็นสัญญาณ snapshot — เหตุผล
เดียวกับที่ `company_contact_line` ต้องประกอบฝั่ง server

**verify เขียวครบ 6 ขั้นทั้ง finance-bc และ report-bc** · smoke ใหม่
`apps/finance-bc/test/smoke/vat-return-print.smoke.mjs` ยิงของจริง: เลือกเดือนที่มีเอกสารจริงมากที่สุด →
พิมพ์ → **เทียบ `document_prints.params` ทีละช่องกับ `GET /vat-returns` ของเดือนเดียวกัน** → ดึง PDF จริง
(363,778 bytes) → พิมพ์ซ้ำแล้วยืนยันว่า `copy_number` เดินต่อและ `source_document_id` เป็นตัวเดิม

**ยังเหลือ**: ข้อ 2 (เครดิตภาษีซื้อ 6 เดือน) · ข้อ 3 (สถานะ "ยื่นแล้ว" + snapshot ต่อเดือน — แต่
`document_prints.params` ของทุกครั้งที่พิมพ์ ทำหน้าที่นี้ได้บางส่วนแล้ว) · ช่องกรณีมีสาขา/ยื่นเพิ่มเติม
บนหัวกระดาษยังพิมพ์เป็นข้อความให้กาเอง
