...
- **D3 · งบการเงิน** (งบดุล/งบกำไรขาดทุน) — ✅ **ฝั่ง JSON implement แล้ว 2026-09-19**
  (`HANDOFF-Financial-Statements.md` §10) · ใช้ `resolveFiscalYear()` ที่ D2 วางไว้ใน `@lib/common` จริง
  ตามที่คาดไว้ และจัดกลุ่มด้วยคอลัมน์ใหม่ `gl_accounts.statement_section` ไม่ใช่ `account_type` เดี่ยว ๆ
  อย่างที่บรรทัดนี้เคยเดา — เหลือแต่ฟอร์มพิมพ์ (P4)
